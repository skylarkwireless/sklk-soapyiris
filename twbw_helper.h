 ...
 * \param trigger true to wait for a trigger event
 ...
 * \param [out] isTrigger true when from a trigger event
...
)
{
    ...
    if (isBurst && burstCount <= numSamps && ((numSamps-burstCount)*packSize) < width)
    {
        numSamps = burstCount;
        burstEnd = true;
        overflow = false;
 ...
 * \param trigger true to wait for a trigger event
 * \param hasSequence request a sequence status packet
 * \param sequence a 16-bit sequence count to embed
 */
...
)
{
    ...
    if (trigger) word0 |= (1 << 26);
    if (hasSequence) word0 |= (1 << 25);
    word1 = (numSamps-1) & 0xffff;
    word1 |= sequence << 16;
    word2 = timeTicks >> 32;
    word3 = timeTicks & 0xffffffff;

    length = width*4 + numSamps*packSize;
}
 ...
 * \param [out] timeError true when a time error occurs
 * \param [out] burstEnd true when this packet ends a burst
 * \param [out] hasSequence true when this packet has a sequence
 * \param [out] sequence the 16-bit sequence supplied by the packer
 * \param [out] seqError a sequence error occurred in the core
 */
...
)
{
    ...
    idTag = (msg[0] >> 16) & 0xff;
    hasSequence = ((msg[0] >> 25) & 0x1) != 0;
    seqError = ((msg[0] >> 24) & 0x1) != 0;

    sequence = short(msg[1] >> 16);
